  ...
  return (
    <section className={`${switchChecked ? "bg-[#19202d]" : "bg-white"} -mt-20 mb-12 h-20 w-[80%] mx-auto rounded-md ${style.colorTrans}`}>
        <div className='flex w-[100%] h-[100%] justify-center items-center'>
            ...
            <Image onClick={() => setModal(true)} className='mx-2 h-6' src='/assets/designs/icon-filter.svg' alt='filter icon' />
            <button onClick={() => setData(search)} className='bg-[#525dcf] p-3 rounded'><Image src='/assets/designs/search-mobile.svg' alt='search' /></button>
        </div>
    </section>
  )
}
