...
const Card = ({job}) => {
  ...
  const style = {
    iconBg: `${job.companyName === "scoot" && "bg-[#e8920f]"} ${job.companyName === "blogr" && "bg-[#e54d29]"} ${job.companyName === "vector" && "bg-[#35353f]"} ${job.companyName === "office lite" && "bg-[#304FC8]"} ${job.companyName === "pad" && "bg-[#101e32]"} ${job.companyName === "creative" && "bg-[#521455]"} ${job.companyName === "pomodoro" && "bg-[#4722c7]"} ${job.companyName === "maker" && "bg-[#22427f]"} ${job.companyName === "coffeeroasters" && "bg-[#f2decd]"} ${job.companyName === "mastercraft" && "bg-[#304fc8]"} ${job.companyName === "crowdfund" && "bg-[#37c890]"} ${job.companyName === "typemaster" && "bg-[#f26818]"}`,
  ...
  return (
    <motion.section whileHover={{ scale: 1.1 }} className={`${switchChecked ? "bg-[#19202d]" : "bg-white"} p-4 w-72 rounded-lg mx-auto cursor-pointer ${style.colorTrans}`}>
      <motion.div animate={{rotate: 360, transition: { duration: 1 }}} className={`${style.iconBg} w-12 h-10 rounded-lg -mt-8 flex justify-center items-center`}><img className="" src={job.jobIcon} alt="" /></motion.div>  
        ...
        <h3 className={`${switchChecked ? "text-white" : "text-black"} my-2 font-bold ${style.colorTrans} `}>{job.jobName}</h3>
